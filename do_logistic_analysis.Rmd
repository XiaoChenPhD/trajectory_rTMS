...
## Adding a variable named "Anxiety" and "biological sex"

Anxiety, 0: no reported comorbidities 1: with anxiety (may include other comorbidities)

Biological sex, 0: biological male, 1: biological female
...
## Doing multinomial logistic regression
...
# defining a function to get p values
...
pValue_extract(model)
exp(coef(model))
CI_lower
CI_upper

# model including medication info
model <- multinom(X_traj_Group ~ AgeMTDay + BaselinePHQ9 + BiologicalSex  + Anxiety + df$`meds_selected$anticonvulsant_use`, data = df, weights = df$Group_Weights)
...
## Calculating the c-statistics and Hosmer-Lemeshow test results
